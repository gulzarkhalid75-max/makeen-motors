...
import type { Category } from "@/app/lib/vehicles";
...
// ── Submitted-vehicles persistence ───────────────────────────

const SUBMITTED_VEHICLES_KEY = "makeen_submitted_vehicles";
...
function SubmittedCard({ v }: { v: StoredVehicle }) {
  const isNew = v.year >= CURRENT_YEAR - 2;
  return (
    <div className="group flex flex-col bg-white/[0.03] backdrop-blur-sm border border-[#C9A356]/18 hover:border-[#C9A356]/40 transition-all duration-500 overflow-hidden">
      {/* Image */}
...
function BrandSection({ vehicles }: { vehicles: StoredVehicle[] }) {
  const byBrand: Record<string, StoredVehicle[]> = {};
  vehicles.forEach(v => {
      ...
      {Object.entries(byBrand).map(([brand, cars]) => (
        <div key={brand}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-6">
            {cars.map(v => <SubmittedCard key={v.id} v={v} />)}
          </div>
        </div>
      ))}
    </div>
...
export default function Inventory() {
  ...
  const [submissions,  setSubmissions]  = useState<StoredVehicle[]>([]);

  useEffect(() => {
    const load = () => {
      try {
        const raw = localStorage.getItem(SUBMITTED_VEHICLES_KEY) ?? "[]";
        const data: StoredVehicle[] = JSON.parse(raw);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-black text-white font-sans">
        ...
        <div className="sticky top-[73px] z-40 bg-black/90 backdrop-blur-xl border-b border-zinc-900/80 px-8 md:px-16 py-5 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative shrink-0">
              <select
                ...
                className="appearance-none h-11 bg-white/[0.04] border border-white/[0.08] pl-4 pr-9 text-[10px] tracking-[0.28em] uppercase text-zinc-400 outline-none focus:border-white/[0.22] transition-all duration-300 cursor-pointer hover:border-white/[0.18] hover:text-white"
          ...
          <p className="text-[10px] tracking-[0.4em] uppercase text-zinc-700">
            {results.length === 0
              ...
              : results.length === VEHICLES.length
        ...
        {submissions.length > 0 && (() => {
          const newVehicles      = submissions.filter(v => v.year >= CURRENT_YEAR - 2);
          const preOwnedVehicles = submissions.filter(v => v.year <  CURRENT_YEAR - 2);
          return (
            <div className="relative border-t border-zinc-900 px-8 md:px-16 py-14 md:py-20">
              <div
              ...
              <div className="relative">
                ...
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-12 md:mb-16">
                  <div>
                    <p className="text-[9px] tracking-[0.55em] uppercase text-zinc-600 mb-2">Customer Submissions</p>
                    <h2 className="text-3xl md:text-5xl font-thin tracking-tight uppercase leading-none">
                      Submitted<br /><span className="text-zinc-500">Vehicles</span>
                    </h2>
                  </div>
                  <span className="text-[10px] tracking-[0.4em] uppercase text-zinc-700">
                    {submissions.length} {submissions.length === 1 ? "vehicle" : "vehicles"}
                  </span>
                </div>
